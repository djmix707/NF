...
def format_result_beautiful(info, is_subscribed, cookie_content, cookie_filename, nftoken_data=None, config=None, html_content=""):
    ...
    for line in lines:
        try:
            final_text += line.encode('utf-8', errors='ignore').decode('utf-8') + "\n"
        except:
            final_text += "[خطأ في الترميز]\n"
    
    return final_text, plan_key
    ...
    for plan, results in results_by_plan.items():
        ...
        for chunk_idx in range(total_chunks):
            ...
            for line in chunk_results:
                try:
                    line_clean = line.encode('utf-8', errors='ignore').decode('utf-8')
                    clean_text_data += line_clean
                except:
                    clean_text_data += "[خطأ في الترميز]\n"
            
            buf = BytesIO()
            buf.write(clean_text_data.encode('utf-8', errors='ignore'))
            buf.seek(0)
...
async def handle_single_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for idx, bundle in enumerate(bundles, 1):
        ...
        if current_time - last_update_time >= update_interval or processed == total_bundles:
            ...
            premium_count = len(results_by_plan["premium"]) // 2
            ...
            progress_msg = format_progress_message(
                ...
                stats['valid'], premium_count, len(results_by_plan["free"]) // 2,
        ...
        if result:
            if result_type == "success":
                target_key = plan_key if (plan_key and plan_key in results_by_plan and plan_key != "free") else "premium"
                if target_key in results_by_plan:
...
"""
        ...
        if results_by_plan["partial"]:
            ...
            all_partial = all_partial.encode('utf-8', errors='ignore').decode('utf-8')
            buf = BytesIO()
            buf.write(all_partial.encode('utf-8', errors='ignore'))
            buf.seek(0)
                        ...
                        if has_required_netflix_cookies(cookies):
                            ...
                            if status_code == 200 and info and any(info.get(f) for f in ["countryOfSignup", "membershipStatus", "localizedPlanName"]):
                                ...
                                if is_sub:
                                    ...
                                    target_key = plan_key if plan_key in results_by_plan and plan_key != "free" else "premium"
